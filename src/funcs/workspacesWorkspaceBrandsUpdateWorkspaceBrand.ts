 ...
 */
export function workspacesWorkspaceBrandsUpdateWorkspaceBrand(
  client: IamClientCore,
  request: operations.UpdateWorkspaceBrandRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.UpdateWorkspaceBrandResponse,
    | errors.ErrorDetails
    | IamClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateWorkspaceBrandRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateWorkspaceBrandBody, {
    explode: true,
  });
  ...
  const path = pathToFunc(
    "/v1/accounts/{accountId}/workspaces/{workspaceId}/brand",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.UpdateWorkspaceBrandResponse$inboundSchema),
