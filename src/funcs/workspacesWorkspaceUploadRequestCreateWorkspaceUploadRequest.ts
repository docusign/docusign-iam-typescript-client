 ...
 */
export function workspacesWorkspaceUploadRequestCreateWorkspaceUploadRequest(
  client: IamClientCore,
  request: operations.CreateWorkspaceUploadRequestRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetWorkspaceUploadRequestResponse,
    | errors.ErrorDetails
    | IamClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateWorkspaceUploadRequestRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.CreateWorkspaceUploadRequestBody, {
    explode: true,
  });
  ...
  const path = pathToFunc(
    "/v1/accounts/{accountId}/workspaces/{workspaceId}/upload-requests",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.GetWorkspaceUploadRequestResponse$inboundSchema),
