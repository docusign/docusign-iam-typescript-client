...
import { GetTokenFromPublicAuthCodeServerList } from "../models/operations/gettokenfrompublicauthcode.js";
 ...
 */
export function authGetTokenFromPublicAuthCode(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.PublicAuthCodeGrantRequestBody$outboundSchema.parse(value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(GetTokenFromPublicAuthCodeServerList[0], {
      charEncoding: "percent",
    })();

  const path = pathToFunc("/oauth/token#FromPublicAuthCode")();
  ...
  const context = {
    ...
    baseURL: baseURL ?? "",
    operationID: "GetTokenFromPublicAuthCode",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.AuthorizationCodeGrantResponse$inboundSchema),
